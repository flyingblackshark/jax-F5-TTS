...
def _reshape_batch_dim_to_heads(tensor, heads):
    ...
    return jax.lax.with_sharding_constraint(
        reshaped_tensor, PartitionSpec("data", "fsdp", "tensor")
    )
        ...
        if attention_kernel == "flash":
            ...
            if num_fsdp_shards > 1:
                ...
                (m_final, l_final, o_final, _, _), _ = jax.lax.scan(
                    ring_scan_body, initial_carry, None, length=num_fsdp_shards - 1
                )
...
def _cudnn_flash_attention(
    query: Array, key: Array, value: Array, heads: int, mesh: Mesh, dpa_layer: Callable
) -> Array:
...
def _apply_attention(
    ...
    if (
        ...
        or use_memory_efficient_attention
    ...
    def __init__(
        ...
        if attention_kernel == "cudnn_flash_te":
            raise NotImplementedError(
                f"{self} has not been tested with {attention_kernel}"
            )
...
class FlaxWanAttention(nnx.Module):
    ...
    def __init__(
        ...
        if attention_kernel == "cudnn_flash_te":
            raise NotImplementedError(
                f"Wan 2.1 has not been tested with {attention_kernel}"
            )

        if attention_kernel in {"flash", "cudnn_flash_te"} and mesh is None:
            raise ValueError(
                f"The flash attention kernel requires a value for mesh, but mesh is {self.mesh}"
            )
        self.dim_head = dim_head
        self.heads = heads
        ...
        qkv_init_kernel = nnx.with_partitioning(
    ...
    def _apply_rope(
        self, xq: jax.Array, xk: jax.Array, freqs_cis: jax.Array
        ...
        xq_out = (
            jnp.stack([jnp.real(xq_out_complex), jnp.imag(xq_out_complex)], axis=-1)
            .reshape(xq.shape)
            .astype(dtype)
        )
        xk_out = (
            jnp.stack([jnp.real(xk_out_complex), jnp.imag(xk_out_complex)], axis=-1)
            .reshape(xk.shape)
            .astype(dtype)
        )
    ...
    def __call__(
        ...
        hidden_states = jax.lax.with_sharding_constraint(
            hidden_states, PartitionSpec("data", "fsdp", "tensor")
        )
        encoder_hidden_states = jax.lax.with_sharding_constraint(
            encoder_hidden_states, PartitionSpec("data", "fsdp", "tensor")
        )
...
class FlaxFluxAttention(nn.Module):
    ...
    def __call__(
        ...
        if encoder_hidden_states is not None:
            ...
            encoder_qkv_proj = encoder_qkv_proj.reshape(B, L, K, H, D).transpose(
                2, 0, 3, 1, 4
            )
        ...
        query_proj = query_proj.transpose(0, 2, 1, 3).reshape(
            query_proj.shape[0], query_proj.shape[2], -1
        )
        key_proj = key_proj.transpose(0, 2, 1, 3).reshape(
            key_proj.shape[0], key_proj.shape[2], -1
        )
        value_proj = value_proj.transpose(0, 2, 1, 3).reshape(
            value_proj.shape[0], value_proj.shape[2], -1
        )
...
class FlaxBasicTransformerBlock(nn.Module):
    ...
    def __call__(
        self, hidden_states, context, deterministic=True, cross_attention_kwargs=None
    ):
        # self attention
...
class FlaxTransformer2DModel(nn.Module):
    ...
    def setup(self):
        ...
        if self.use_linear_projection:
            self.proj_in = nn.Dense(
                inner_dim,
                kernel_init=nn.with_logical_partitioning(
                    nn.initializers.lecun_normal(), ("embed", "hidden")
                ),
                dtype=self.dtype,
                param_dtype=self.weights_dtype,
                precision=self.precision,
        ...
        if self.use_linear_projection:
            self.proj_out = nn.Dense(
                inner_dim,
                kernel_init=nn.with_logical_partitioning(
                    nn.initializers.lecun_normal(), ("hidden", "embed")
                ),
                dtype=self.dtype,
                param_dtype=self.weights_dtype,
                precision=self.precision,
    ...
    def __call__(
        ...
        hidden_states = nn.with_logical_constraint(
            hidden_states, self.hidden_state_axis_names
        )
...
class FlaxF5Attention(nnx.Module):
    ...
    def apply_rotary_pos_emb(self, t, freqs, scale=1, decoder_segment_ids=None):
        ...
        t = (t * jnp.cos(freqs) * scale) + (
            self.rotate_half(t) * jnp.sin(freqs) * scale
        )
    ...
    def __call__(
        ...
        if rope is not None:
            ...
            query_proj = jnp.reshape(
                query_proj, (query_proj.shape[0], -1, self.heads, self.dim_head)
            ).transpose(0, 2, 1, 3)
            key_proj = jnp.reshape(
                key_proj, (key_proj.shape[0], -1, self.heads, self.dim_head)
            ).transpose(0, 2, 1, 3)
            value_proj = jnp.reshape(
                value_proj, (value_proj.shape[0], -1, self.heads, self.dim_head)
            ).transpose(0, 2, 1, 3)
            query_proj = self.apply_rotary_pos_emb(
                query_proj, freqs, q_xpos_scale, decoder_segment_ids
            )
            key_proj = self.apply_rotary_pos_emb(
                key_proj, freqs, k_xpos_scale, decoder_segment_ids
            )

        hidden_states = self.attention_op.apply_attention(
            jnp.reshape(
                query_proj.transpose(0, 2, 1, 3),
                (query_proj.shape[0], -1, self.heads * self.dim_head),
            ),
            jnp.reshape(
                key_proj.transpose(0, 2, 1, 3),
                (query_proj.shape[0], -1, self.heads * self.dim_head),
            ),
            jnp.reshape(
                value_proj.transpose(0, 2, 1, 3),
                (query_proj.shape[0], -1, self.heads * self.dim_head),
            ),
            decoder_segment_ids=decoder_segment_ids,
        )
        ...
        hidden_states = jnp.reshape(
            hidden_states, (hidden_states.shape[0], -1, self.heads * self.dim_head)
        )
