...
def get_mel(y, n_mels=100,n_fft=1024,win_size=1024,hop_length=256,fmin=0,fmax=None,clip_val=1e-7,sampling_rate=24000):
    ...
    # Audax uses norm=None, htk=True by default (matching torchaudio)
    mel_filters = librosa.filters.mel(sr=sampling_rate, n_fft=n_fft, n_mels=n_mels, fmin=fmin, fmax=fmax, norm=None, htk=True)
    ...
    # Audax/Torchaudio uses periodic window
    window = jnp.hanning(win_size + 1)[:-1]
    
    # jax.scipy.signal.stft defaults to scaling='spectrum' which divides by sum(window).
    # Audax / Torchaudio does NOT scale.
    # So we need to multiply by sum(window) to match.
