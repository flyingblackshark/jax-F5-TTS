...
# 需要先安装: pip install lameenc
# 或者使用ffmpeg-python

@app.post("/generate_realtime_stream")
async def generate_realtime_audio_stream(request: AudioGenerationRequest):
    """Generate audio with real-time MP3 streaming."""
    try:
        # 解码音频数据（与之前相同）
        ...
        async def realtime_mp3_generator():
            """实时MP3流生成器"""
            import subprocess
            import threading
            ...
            try:
                ...
                async def feed_audio():
                    async for chunk in generate_audio_stream(
                        ...
                        # 解码音频块并发送到ffmpeg
                        audio_bytes = base64.b64decode(chunk.audio_chunk)
                        process.stdin.write(audio_bytes)
                        process.stdin.flush()
                        
                        # 检查是否有输出可用
                        while not output_queue.empty():
                            mp3_chunk = output_queue.get()
                    ...
                    while True:
                        try:
                            mp3_chunk = output_queue.get(timeout=1.0)
                            if mp3_chunk is None:
                                break
                            yield mp3_chunk
                        except:
                ...
                async for mp3_chunk in feed_audio():
                    yield mp3_chunk
        ...
        max_logging.error(f"Error in realtime stream: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/generate_realtime_opus_stream")
async def generate_realtime_opus_stream(request: AudioGenerationRequest):
    """Generate audio with real-time Opus streaming."""
    try:
        # 解码音频数据（与之前相同）
        audio_data = base64.b64decode(request.ref_audio_base64)
        audio_stream = io.BytesIO(audio_data)
        ...
        async def realtime_opus_generator():
            """实时Opus流生成器"""
            import subprocess
            import threading
            from queue import Queue
            ...
            ffmpeg_cmd = [
                ...
                '-ac', '1',  # 单声道
                '-i', 'pipe:0',  # 从stdin读取
                '-c:a', 'libopus',  # 使用Opus编解码器
                '-b:a', '64k',  # 比特率（Opus推荐64k用于语音）
                '-vbr', 'on',  # 启用可变比特率
                '-compression_level', '10',  # 最高压缩级别
                '-frame_duration', '20',  # 20ms帧长度（适合实时流）
            ...
            try:
                ...
                async def feed_audio():
                    ...
                    while True:
                        try:
                            opus_chunk = output_queue.get(timeout=1.0)
                            if opus_chunk is None:
                                break
                            yield opus_chunk
                        except:
                ...
                async for opus_chunk in feed_audio():
                    yield opus_chunk
        ...
        return StreamingResponse(
            realtime_opus_generator(),
            media_type="audio/opus",
            headers={
                "Content-Disposition": "attachment; filename=generated_audio.opus",
                "Cache-Control": "no-cache",
                "Transfer-Encoding": "chunked"
            }
        )
        ...
        max_logging.error(f"Error in realtime opus stream: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))